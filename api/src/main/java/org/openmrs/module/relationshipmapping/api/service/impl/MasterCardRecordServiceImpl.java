...
import org.openmrs.module.relationshipmapping.model.MasterCardRecord;
...
public class MasterCardRecordServiceImpl extends BaseOpenmrsService implements MasterCardRecordService {
    ...
    public void processUnmappedMastercardRecords(){
        Date previousExecutionDate = new Date(0);
        MasterCardRecordMappingExecutionCycle previousCycle = getLatestExecutionCycle();
        if(previousCycle != null){
            previousExecutionDate = previousCycle.getLatestRecordCreationDate();
        }

        mapMastercardRecords(previousExecutionDate);
    }

    public void mapMastercardRecords(Date previousExecutionDate){
        if(isTaskRunning){
            return;
        }
        isTaskRunning = true;
        try {
            ...
            List<MasterCardObsGroup> groups = masterCardRecordDao.getNewMasterCardObsGroups(previousExecutionDate);
//            log.info("Mastercard Relationship mapping found ["+groups.size()+"] groups created since ["+sf.format(previousExecutionDate)+"]");
            ...
            for (MasterCardObsGroup group : groups) {
                ...
                Patient indexPatient = Context.getPatientService().getPatient(group.getIndexCaseId());
                MasterCardRecord masterCardRecord = new MasterCardRecord();
                masterCardRecord.setIndexPatient(indexPatient);
                masterCardRecord.setObsDateTime(group.getObsDateTime());
                List<MasterCardObsGroupMember>members = masterCardRecordDao.getMasterCardObsGroupMembers(group.getObsId());
                ...
                || isParent(masterCardRecord)) {
                    ...
                    if (StringUtils.isNotBlank(masterCardRecord.getNid())) {
                        //search for existing person by NID. if not found,create a new person and relationship.
                        // If found, make it the relationship person

                        List<Patient> contacts = Context.getPatientService().getPatients(null, masterCardRecord.getNid(), identifierTypes, true);
                        ...
                        if (contacts.size() == 1) {
                            ...
                            // Check if there exists a relationship of the same type already before creating a new one.
                            ...
                            List<Relationship> existingRelationships = Context.getPersonService().getRelationships(contacts.get(0), indexPatient, relationshipType);
                            if (existingRelationships.size() == 0) {
                                ...
                                //ToDo: How about obs? Update them if mastercard is newer and has different value?
...
//                                        "] and contact person ["+contacts.get(0).getId()+"]");
                            ...
                            //Patient with the NID in the mastercard record could not be uniquely identified
                            //Search for existing related person with matching names as the mastercard.
                            // If not found, create a new person record, Create a relationship with index case, and then create Obs.
                            Person contactPerson = searchForExistingRelatedPersonWithSimilarNames(masterCardRecord);
                            if (contactPerson == null) {
                                contactPerson = createNewRelationshipPerson(masterCardRecord, masterCardRecordMappingLog, executionCycle);
...
//                                    "] and contact person ["+contactPerson.getPersonId()+"]");
                ...
                if (masterCardRecordMappingLog.getRelationship() != null) {
                ...
                executionCycle.setLatestRecordCreationDate(group.getCreationDate());
            }
...
//            log.info("Mastercard Relationship mapping: Completed Relationship mapping session: "+executionCycle.toString());
    ...
    private boolean isBelow15AndNonTestedPositiveFilho(MasterCardRecord masterCardRecord){
        ...
        return masterCardRecord.getRelationshipTypeConceptId() == 23707
                && contactBirthDate.after(cutOffBirthDate)
                && masterCardRecord.getHivTest() != 703
                && masterCardRecord.getHivTest() != 664;
    }

    private boolean isParent(MasterCardRecord masterCardRecord){
        return masterCardRecord.getRelationshipTypeConceptId() == 970 || masterCardRecord.getRelationshipTypeConceptId() == 971;
    }
    ...
    private void createPersonObs(MasterCardRecord masterCardRecord,
        ...
        if(masterCardRecord.getChildAtRiskTreatment() > 0) {
            ...
            Concept childAtRiskTreatmentValueCodedConcept = Context.getConceptService().getConcept(masterCardRecord.getChildAtRiskTreatment());
